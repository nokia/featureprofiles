...
// Ensure mTLS is enabled on the dut for this test to succeed.
func TestAccountzAuthenFailMulti(t *testing.T) {
	...
	startTime := time.Now()
	for _, tc := range sshPasswordTestCases {
		record := acctz.SendFailCliLogin(t, dut, tc.username, tc.password, acctzpb.AuthnDetail_AUTHN_TYPE_PASSWORD)
		records = append(records, record)
	}
	record := acctz.SendFailCliLogin(t, dut, username, password, acctzpb.AuthnDetail_AUTHN_TYPE_SSHKEY)
	records = append(records, record)
	record = acctz.SendFailCliLogin(t, dut, username, password, acctzpb.AuthnDetail_AUTHN_TYPE_SSHCERT)
	records = append(records, record)
	record = acctz.SendFailShellLogin(t, dut)
	records = append(records, record)
	record = acctz.SendFailTlsLogin(t, dut)
	records = append(records, record)
